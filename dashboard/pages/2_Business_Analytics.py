...
k2.metric("Total Market Volume", f"${total_vol/1e6:.1f}M",  f"{len(filtered):,} transactions")
k3.metric("Price Volatility (CV)", f"{cv:.1f}%",            "Stable" if cv < 35 else "High")
k4.metric("Top Neighborhood",    top_hood,                  f"${top_hood_med:,.0f} median")
k5.metric("Luxury Premium",      f"+{premium_pct:.0f}%",    "vs. market median")
    ...
    with st.spinner("Computing prediction…"):
        try:
            ...
            if resp.status_code == 200:
                ...
                r3.metric("Position vs. Median", f"{mkt_pos:+.1f}%", "above median" if mkt_pos >= 0 else "below median")
