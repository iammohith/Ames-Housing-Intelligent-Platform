...
# ── Executive KPI Row ──────────────────────────────────────────────────────
st.markdown(
    '<div class="section-header">Executive Summary KPIs</div>', unsafe_allow_html=True
)
...
top_hood = (
    filtered.groupby("Neighborhood")["SalePrice"].median().idxmax()
    if len(filtered) > 0
    else "N/A"
)
top_hood_premium = filtered.groupby("Neighborhood")["SalePrice"].median().max()
market_premium = (
    ((top_hood_premium - median_price) / median_price * 100) if median_price > 0 else 0
)
...
# ── Market Segmentation ───────────────────────────────────────────────────
st.markdown(
    '<div class="section-header">🏙️ Market Segmentation Analysis</div>',
    unsafe_allow_html=True,
)
...
st.markdown(
    '<div class="section-header">💰 Price Distribution & Market Concentration</div>',
    unsafe_allow_html=True,
)
...
with col_insight:
    ...
    iqr = q75 - q25
    luxury_share = (
        len(filtered[filtered["SalePrice"] > q75]) / max(len(filtered), 1) * 100
    )
    st.markdown(
        f"""
...
st.markdown(
    '<div class="section-header">🏘️ Neighborhood ROI & Value Concentration</div>',
    unsafe_allow_html=True,
)
...
with col_treemap:
    ...
    fig_tree.update_layout(
        height=420, font=dict(family="Inter"), margin=dict(t=20, b=0)
    )
    fig_tree.update_traces(
        textinfo="label+value", texttemplate="%{label}<br>$%{value:.1f}M"
    )
    st.plotly_chart(fig_tree, use_container_width=True)

# ── Temporal Market Cycle Analysis ───────────────────────────────────────
st.markdown(
    '<div class="section-header">📅 Market Cycle & Temporal Demand Analysis</div>',
    unsafe_allow_html=True,
)
...
with col_line:
    yr_trend = (
        filtered.groupby("Yr Sold")["SalePrice"].agg(["median", "count"]).reset_index()
    )
    fig_trend = go.Figure()
    fig_trend.add_trace(
        go.Scatter(
...
with col_heat:
    temporal = (
        filtered.groupby(["Yr Sold", "Mo Sold"])["SalePrice"].mean().reset_index()
    )
...
st.markdown(
    '<div class="section-header">🔬 Value Driver Intelligence — What Moves Price?</div>',
    unsafe_allow_html=True,
)
...
with col_scatter:
    ...
    if qual_col:
        sample_df = filtered.sample(min(1000, len(filtered)), random_state=42).dropna(
            subset=[qual_col, "SalePrice"]
        )
        fig5 = px.scatter(
            sample_df,
            x=qual_col,
...
st.markdown(
    '<div class="section-header">🏆 Predictive Model Leaderboard</div>',
    unsafe_allow_html=True,
)
...
try:
    ...
    if resp.status_code == 200:
        ...
        if models:
            ...
            with col_gauge:
                best_r2 = (
                    model_df["test_r2"].max() if "test_r2" in model_df.columns else 0.92
                )
                fig_gauge = go.Figure(
                    go.Indicator(
                        ...
                        value=best_r2,
                        title={
                            "text": "Best Model R²<br><span style='font-size:0.8em'>Test Set Performance</span>"
                        },
                ...
                fig_gauge.update_layout(
                    height=280, font=dict(family="Inter"), margin=dict(t=40, b=0)
                )
...
st.markdown(
    '<div class="section-header">🎛️ Strategic Pricing Simulator — What-If Analysis</div>',
    unsafe_allow_html=True,
)
st.markdown(
    "Stress-test pricing assumptions by adjusting property attributes. Powered by the XGBoost champion model."
)
...
with sim1:
    sim_qual = st.slider(
        ...
        10,
        7,
        help="Rates the overall material and finish quality",
    )
    ...
    try:
        ...
        resp = requests.post(
            ...
            json={
                ...
                "gr_liv_area": sim_area,
                "year_built": sim_year,
                "total_bathrooms": sim_bath,
                "neighborhood": sim_hood,
                "garage_area": sim_garage,
            },
            timeout=10,
        )
        if resp.status_code == 200:
            ...
            if shap and "error" not in shap:
                st.markdown(
                    "**📊 SHAP Value Decomposition — Marginal Feature Contributions:**"
                )
                shap_df = pd.DataFrame(
                    list(shap.items()), columns=["Feature", "SHAP Value"]
                )
            ...
            st.error(
                "Prediction unavailable — run the full pipeline first to train models."
            )
