...
INTENT_PATTERNS = {
    "neighborhood": {
        "keywords": [
            ...
            "idotrr", "price", "sale", "market"
        ],
        "patterns": [
            r"which.*neighborhood.*(?:best|most|popular|expensive|affordable|highest|top)",
            r"neighborhood.*(?:comparison|stats|analysis|price|sale)",
            r"(?:best|worst|highest|top).*neighborhood",
...
def classify_query(query: str) -> Tuple[QueryIntent, float]:
    ...
    for intent, config in INTENT_PATTERNS.items():
        ...
        # Cap keyword matches (1 or 2 keywords is a strong signal)
        scores[intent] += min(keyword_score / 2.0, 1.0) * 0.4 * weight
    ...
    for intent, config in INTENT_PATTERNS.items():
        ...
        for pattern in config["patterns"]:
            if re.search(pattern, q_lower, re.IGNORECASE):
                pattern_score += 1
        weight = config.get("weight", 1.0)
        # Matching even 1 pattern is a very strong signal
        scores[intent] += min(pattern_score, 1.0) * 0.6 * weight
