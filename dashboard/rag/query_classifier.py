...
def classify_query(query: str) -> Tuple[QueryIntent, float]:
    ...
    # Normalize confidence: max score / total possible points
    max_score = max(scores.values())
    total_possible = (len(INTENT_PATTERNS[max([k for k, v in scores.items() if v == max_score][0])]["keywords"]) * 0.4 + 
                      len(INTENT_PATTERNS[max([k for k, v in scores.items() if v == max_score][0])]["patterns"]) * 0.6)
