...
class RetrievalResult(TypedDict):
    context:    str           # assembled prompt context
    intent:     str           # routed namespace
    confidence: float         # routing confidence score
    sources:    list[dict]    # [{"chunk_id", "doc_id", "namespace", "score", "rank"}]
    fallback:   bool          # whether extractive fallback fired
    trace_id:   str           # correlates to dashboard WebSocket event
...
def retrieve_context(query: str, top_k: int = 5) -> RetrievalResult:
    ...
    # Default fallback result structure
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"ChromaDB collection not found: {e}. Knowledge base may need rebuilding.", trace_id=trace_id)
            fallback_text = _fallback_context(query)
            if fallback_text and not fallback_text.startswith("[FALLBACK]"):
                fallback_result["context"] = f"[⚠️ FALLBACK - Knowledge base empty] {fallback_text}"
            else:
                fallback_result["context"] = fallback_text
        ...
        logger.info(f"Query intent: {intent} (confidence: {confidence:.2f})", trace_id=trace_id)
        fallback_result["intent"] = intent
        fallback_result["confidence"] = confidence
        
        try:
            dense_results = collection.query(query_texts=[query], n_results=initial_k)
        except Exception as e:
            logger.warning(f"Dense retrieval failed: {e}. Using fallback.", trace_id=trace_id)
            fallback_result["context"] = _fallback_context(query)
            return fallback_result
        ...
        if confidence > 0.3 and intent != "general" and intent in INTENT_TO_TITLES:
            ...
            try:
                ...
                logger.info(f"Found {len(intent_docs)} documents matching intent '{intent}'", trace_id=trace_id)
            except Exception as filter_err:
                # If metadata filtering fails, log and continue with dense results
                logger.warning(f"ChromaDB metadata filtering failed: {filter_err}. Using dense results only.", trace_id=trace_id)
        ...
        if not dense_docs:
            logger.warning(f"No dense results for query. Returning fallback.", trace_id=trace_id)
            fallback_result["context"] = _fallback_context(query)
            return fallback_result
        ...
        # If we are extremely confident in the intent, we can aggressively filter out irrelevant documents
        # to prevent the small 250M parameter LLM from getting distracted by them.
        if intent_docs and confidence >= 0.7:
            final_chunks = intent_docs[:top_k]
        else:
        ...
        if final_chunks:
            ...
            try:
                ...
                logger.warning(f"Failed to inject live state: {inject_err}", trace_id=trace_id)
            # ─────────────────────────────────────────
        ...
        logger.warning(f"retrieve_context failed: {e}", trace_id=trace_id)
