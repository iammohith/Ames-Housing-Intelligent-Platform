...
class BaseAgent(ABC, Generic[InputT, OutputT]):
    ...
    @abstractmethod
    async def execute(self, input_data: InputT, df: pd.DataFrame = None) -> tuple[OutputT, pd.DataFrame]:
        """Core agent logic — implemented by each agent."""
        ...

    async def run(self, input_data: InputT, df: pd.DataFrame = None) -> tuple[OutputT, pd.DataFrame]:
        """Wrapper: timing, logging, metrics, event broadcasting."""
        await self.emit(AgentStatus.STARTED, f"{self.name} initialising")
        start_time = time.time()
        try:
            result, df_out = await self.execute(input_data, df=df)
