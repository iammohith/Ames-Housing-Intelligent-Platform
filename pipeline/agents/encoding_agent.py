...
class EncodingAgent(BaseAgent):
    ...
    async def execute(self, input_data: Dict[str, Any], df: pd.DataFrame = None) -> tuple[EncodingOutput, pd.DataFrame]:
        if df is None:
            raise ValueError("No DataFrame provided to EncodingAgent")
        ...
        # Temporal split indices (Single source of truth — before dropping Yr Sold)
        train_mask = df["Yr Sold"].isin([2006, 2007, 2008])
        
        # Save original identifying columns for anomaly agent metadata
        df["PID_orig"] = df.get("PID", df.index.astype(str))
        df["Neighborhood_orig"] = df.get("Neighborhood", "Unknown")

        # Drop non-feature columns: Order is an index, PID is an identifier,
        # artifact_flag is a cleaning-stage tag — none should be model features
        cols_to_drop = [c for c in DROP_COLS + ["artifact_flag"] if c in df.columns]
        if cols_to_drop:
        ...
        # Local-safe artifacts path
        base_artifacts = os.getenv("ARTIFACTS_DIR", "./artifacts")
        artifact_dir = os.path.join(base_artifacts, "encoders", run_id)
        ...
        if ohe_cols_present:
            ...
            ohe_feature_names = list(ohe.get_feature_names_out(ohe_cols_present))
        ...
        ohe_features = len(ohe_feature_names)
        await self.emit(
            AgentStatus.PROGRESS,
        ...
        if remaining_str:
            ...
            await self.emit(
                AgentStatus.WARNING, 
                f"Dropped {len(remaining_str)} unused categorical columns: {', '.join(remaining_str)}"
            )
        ...
        for col in LOG_TRANSFORM_COLS:
            if col in df.columns:
                # Use np.log1p for numerical stability with zeros
        ...
        exclude_from_scaling = ["SalePrice", "log_SalePrice", "Yr Sold", "PID", "Order"]
        scale_cols = [
            c
            for c in numeric_cols
            if c not in exclude_from_scaling and not c.startswith("log_") and c not in ohe_feature_names
        ]
