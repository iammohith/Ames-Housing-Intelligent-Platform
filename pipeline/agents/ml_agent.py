...
from core.schemas import AgentStatus, MLTrainingOutput, ModelMetrics, ModelResult
from core.metrics import model_rmse, model_r2, model_mae
...
class MLAgent(BaseAgent):
    ...
    async def execute(self, input_data) -> MLTrainingOutput:
        ...
        exclude = {"SalePrice", "log_SalePrice", "Yr Sold", "anomaly_flagged", "anomaly_severity"}
        ...
        models_config = {
            "ridge": RidgeCV(alphas=[0.1, 1.0, 10.0, 100.0], cv=5, scoring="neg_root_mean_squared_error"),
            "xgboost": XGBRegressor(
                n_estimators=500, learning_rate=0.05, max_depth=4,
                min_child_weight=3, subsample=0.8, colsample_bytree=0.8,
                reg_alpha=0.01, reg_lambda=1.0, random_state=42, n_jobs=-1,
                verbosity=0,
            ),
        ...
        for name, model in models_config.items():
            ...
            if name == "xgboost":
                ...
                await self.emit(AgentStatus.PROGRESS, f"XGBoost stopped at iter {best_iter} (early stopping)")
            elif name == "lightgbm":
                model.fit(
                    X_train, y_train,
                    eval_set=[(X_val, y_val)],
        ...
        await self.emit(
            ...
            metric_key="best_test_r2", metric_value=best_r2,
        )

        # Update Prometheus metrics
        for result in model_results:
            model_rmse.labels(model_name=result.model_name, split="val").set(result.val_metrics.rmse_dollars)
            model_rmse.labels(model_name=result.model_name, split="test").set(result.test_metrics.rmse_dollars)
            model_r2.labels(model_name=result.model_name, split="test").set(result.test_metrics.r2)
            model_mae.labels(model_name=result.model_name, split="test").set(result.test_metrics.mae_dollars)

        # SHAP analysis
        shap_artifacts = []
        try:
            import shap
            await self.emit(AgentStatus.PROGRESS, f"Computing SHAP values on {len(X_test)} test properties...")
        ...
        try:
            ...
            await self.emit(AgentStatus.PROGRESS, f"Registering {best_model_name} in MLflow Registry ✓")
        except Exception as e:
    ...
    def _evaluate(self, model, X, y_log, split: str, use_log: bool) -> ModelMetrics:
        ...
        return ModelMetrics(
            ...
            mape=round(float(mean_absolute_percentage_error(y_actual_safe, y_pred)) * 100, 2),
