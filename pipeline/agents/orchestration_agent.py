...
class OrchestrationAgent(BaseAgent):
    ...
    async def _persist_run(self, results: dict):
        ...
        try:
            ...
            # Register UUID and JSON adapters; adapt Python lists → Postgres ARRAY
            ...
            rows = getattr(ml_result, "row_count", 0) if ml_result else 0
            cur.execute(
                "INSERT INTO pipeline_runs (run_id, dataset_hash, status, rows_out) "
            ...
            if ml_result and hasattr(ml_result, "model_results"):
                for mr in ml_result.model_results:
                    cur.execute(
                        ...
                        "(run_id,model_name,val_rmse,test_rmse,test_r2,test_mae,test_mape,is_best) "
            ...
            # ── Anomaly log ──────────────────────────────────────────────────
            # CRITICAL: psycopg2 cannot auto-adapt Python list → Postgres TEXT[].
            # Convert methods list to a Postgres ARRAY literal: '{a,b}' string.
            anomaly_result = results.get("anomaly_agent")
            if anomaly_result and hasattr(anomaly_result, "anomaly_report"):
                for record in anomaly_result.anomaly_report.flagged_records:
                    # Convert Python list → Postgres TEXT[] literal
                    methods_array = "{" + ",".join(record.methods) + "}"
                    cur.execute(
                        "INSERT INTO anomaly_log "
                        "(run_id, pid, neighborhood, methods, isolation_score, severity, anomalous_features) "
                        "VALUES (%s,%s,%s,%s::text[],%s,%s,%s)",
                        (
                            self.run_id,
                            record.pid,
                            record.neighborhood,
            ...
            if schema_result and hasattr(schema_result, "schema_report"):
                for col in schema_result.schema_report.columns:
                    cur.execute(
                        ...
                        "(run_id, column_name, data_type, null_rate, is_structural_na) "
                        "VALUES (%s,%s,%s,%s,%s)",
                        (
                            self.run_id,
                            col.name,
            ...
            await self.emit(
                AgentStatus.PROGRESS,
                f"DB persistence complete: {len(anomaly_result.anomaly_report.flagged_records) if anomaly_result and hasattr(anomaly_result, 'anomaly_report') else 0} anomalies, "
                f"{len(schema_result.schema_report.columns) if schema_result and hasattr(schema_result, 'schema_report') else 0} schema columns persisted",
            )
        except Exception as e:
            import structlog
            structlog.get_logger().error(
                "DB persistence failed in _persist_run",
                error=str(e),
                run_id=self.run_id,
                exc_info=True,
            )
            await self.emit(AgentStatus.WARNING, f"DB persistence failed: {e}")
