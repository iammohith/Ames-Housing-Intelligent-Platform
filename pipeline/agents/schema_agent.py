...
class SchemaAgent(BaseAgent):
    ...
    async def execute(self, input_data: Dict[str, Any], df: pd.DataFrame = None) -> tuple[SchemaOutput, pd.DataFrame]:
        if df is None:
            raise ValueError("No DataFrame provided to SchemaAgent")
        ...
        for col in df.columns:
            ...
            if col in NUMERIC_CONTINUOUS:
                ...
                num_continuous += 1
            elif col in NUMERIC_DISCRETE:
                ctype = ColumnType.NUMERIC_DISCRETE
                num_discrete += 1
            elif col in ORDINAL_CATEGORICAL:
        ...
        if "SalePrice" in df.columns:
            ...
            sp_valid = False
            await self.emit(AgentStatus.WARNING, "SalePrice column missing after renaming!")
